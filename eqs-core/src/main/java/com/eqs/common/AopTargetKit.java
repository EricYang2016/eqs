...
public class AopTargetKit {
	...
	public static boolean isDoradoProxy(Object proxy){
		String className = proxy.getClass().getName();
		return className.contains("_$$_");
	}

	public static Class<?> getDoradoProxyTargetClass(Object proxy){
		if(isDoradoProxy(proxy)){
			String className = proxy.getClass().getName();
			int index = className.indexOf("_$$_");
			className = className.substring(0,index);
			return ClassKit.newInstance(className).getClass();
		}
